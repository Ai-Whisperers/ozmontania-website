...
export default function TiendaPage() {
  const items = content.tienda.items as any[]
    ...
    <>
      <section className="pt-32 pb-16">
        <div className="container-art text-center">
          <h1 className="section-title mb-4">{content.tienda.title}</h1>
          <p className="section-subtitle mx-auto">{content.tienda.description}</p>
        </div>
      </section>
              ...
              <div key={item.id} className="glass-panel overflow-hidden group transition-all duration-300 hover:border-amber-500/30">
                ...
                <div className="aspect-square bg-gradient-to-br from-zinc-800 to-zinc-900 flex items-center justify-center">
                  <div className="text-center p-6">
                    <span className="text-4xl mb-2 block">{item.type === 'print' ? '🖼️' : '👕'}</span>
                ...
                <div className="p-5">
                  <div className="flex items-center justify-between mb-2">
                    <h3 className="font-serif font-bold text-zinc-100">{item.name}</h3>
                  ...
                  {item.sizes && item.sizes.length > 0 && (
                    <div className="flex flex-wrap gap-1.5 mb-4">
                      {item.sizes.map((size: string) => (
                        <button
                          key={size}
                          onClick={() => handleSize(item.id, size)}
                  ...
                  <button
                    onClick={() => buyViaWhatsApp(item)}
                    className="w-full py-2.5 bg-green-600 hover:bg-green-500 text-white rounded-xl text-sm font-medium transition-all flex items-center justify-center gap-2"
                  >
                  ...
                  <p className="text-center text-zinc-600 text-[10px] mt-2">
                    Envíos a todo Paraguay. Prints: papel algodón fine art.
                  </p>
                </div>
              </div>
            ))}
